class_name SubmitsVictory extends Node
...
func on_pressed():
	...
	var success := await Leaderboards.post_guest_score(SmartQuiver.default_leaderboard_id, State.victory_time, name_input.text)
